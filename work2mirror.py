...
for project in manifest.projects.itervalues():
    ...
    if project.worktree and os.path.exists( project.worktree ):
        ...
        shutil.move( project.worktree, newworktree )

        # 删除工作区的目录树
        dirtree = project.worktree
        while True:
            dirtree_parent = os.path.dirname(dirtree)
            if len(os.listdir(dirtree_parent)) != 0 or dirtree_parent == cwd:
                break
            else:
                dirtree = dirtree_parent
                print "Remove empty directroy: %s" % (dirtree_parent)
                os.rmdir(dirtree_parent)
